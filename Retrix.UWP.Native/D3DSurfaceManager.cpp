...
IDirect3DSurface^ D3DSurfaceManager::CreateWriteableD3DSurface(CanvasDevice^ device, unsigned int width, unsigned int height)
{
...
D3DSurfaceMap^ D3DSurfaceManager::Map(CanvasDevice^ device, IDirect3DSurface^ surface)
{
	ComPtr<ID3D11Device> d3dDevice;
	__abi_ThrowIfFailed(GetDXGIInterface(device, d3dDevice.GetAddressOf()));
	...
	return ref new D3DSurfaceMap(mappedResource);
}

void D3DSurfaceManager::Unmap(CanvasDevice^ device, IDirect3DSurface^ surface)
{
	ComPtr<ID3D11Device> d3dDevice;
	__abi_ThrowIfFailed(GetDXGIInterface(device, d3dDevice.GetAddressOf()));
